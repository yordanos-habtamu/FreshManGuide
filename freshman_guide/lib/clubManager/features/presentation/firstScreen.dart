...
class ClubsOverviewScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Fetch clubs when the screen loads
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (clubProvider.allClubs.isEmpty && !clubProvider.isLoading) {
        clubProvider.fetchAllClubs();
      }
    });
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16.0),
            // Club Icons Row (Dynamic based on fetched clubs)
            clubProvider.isLoading
                ...
                : clubProvider.errorMessage != null
                    ...
                    : Row(
                        mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                        children: clubProvider.allClubs.take(4).map((club) {
                          return Column(
                            children: [
                              Icon(
            ...
            const SizedBox(height: 16.0),
            // Stats Sections (Dynamic based on club data)
            Expanded(
                  ...
                  : clubProvider.errorMessage != null
                      ...
                      : Column(
                          children: [
                            _buildStatCard(
                              'Total Clubs',
                              clubProvider.allClubs.length.toString(),
                            ),
                            _buildStatCard(
                              'Total Members',
                              clubProvider.allClubs
                                  .fold<int>(
                                      0,
                                      (sum, club) =>
                                          sum + (club.members?.length ?? 0))
                                  .toString(),
                            ),
                            _buildStatCard(
                                'Successful Events', '100'), // Placeholder
                            _buildStatCard(
                                'Member Alumni', '100'), // Placeholder
                          ],
                        ),
            ),
          ],
  ...
  // Helper to map club names to icons
